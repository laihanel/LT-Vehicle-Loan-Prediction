...
# Using opendatasets package to download data and save in the working directory.
'''
...
def split_data(inpath, target_name, outpath_train, outpath_test, test_size):
...
#with stratify we preserve the same proportion of defaults inside the data base
    training = pd.concat([X_train, y_train], axis=1)
    testing = pd.concat([X_test, y_test], axis=1)
    training.to_csv(outpath_train)
    testing.to_csv(outpath_test)
    return training, testing

inpath = 'lt-vehicle-loan-default-prediction/train.csv'
target_name = 'loan_default'
outpath_train = 'lt-vehicle-loan-default-prediction/final_train.csv'
outpath_test = 'lt-vehicle-loan-default-prediction/final_test.csv'
training, testing = split_data(inpath, target_name, outpath_train, outpath_test, test_size=0.3)


print(training)


training.shape  # 70% of the total (training+testing)

testing.shape # 30% of the total (training+testing)


# training
# The frequency of defaults
yes = training.loan_default.sum()
no = len(training.loan_default)-yes
Default_perc_training = round(yes/len(training)*100, 1)

print(Default_perc_training)

# testing
# The frequency of defaults
yes = testing.loan_default.sum()
no = len(testing.loan_default)-yes
Default_perc_test = round(yes/len(testing)*100, 1)

print(Default_perc_test)

# Understand the variables
variables = pd.DataFrame(columns=['Variable','Number of unique values','Values'])

for i, var in enumerate(training.columns):
    variables.loc[i] = [var, training[var].nunique(), training[var].unique().tolist()]
...
#Create Age and Disbursal_months (Training)
training['DisbursalDate'] = pd.to_datetime(training['DisbursalDate'], format = "%d-%m-%y",infer_datetime_format=True)
training['Date.of.Birth'] = pd.to_datetime(training['Date.of.Birth'], format = "%d-%m-%y",infer_datetime_format=True)


training['Date.of.Birth'] = training['Date.of.Birth'].where(training['Date.of.Birth'] < pd.Timestamp('now'), training['Date.of.Birth'] -  np.timedelta64(100, 'Y'))

training['Age']=(pd.Timestamp('now')-training['Date.of.Birth']).astype('<m8[Y]').astype(int)
training['Disbursal_months'] = ((pd.Timestamp('now') - training['DisbursalDate'])/np.timedelta64(1,'M')).astype(int)

#Create Age and Disbursal_months (Testing)

testing['DisbursalDate'] = pd.to_datetime(testing['DisbursalDate'], format = "%d-%m-%y",infer_datetime_format=True)
testing['Date.of.Birth'] = pd.to_datetime(testing['Date.of.Birth'], format = "%d-%m-%y",infer_datetime_format=True)


testing['Date.of.Birth'] = testing['Date.of.Birth'].where(testing['Date.of.Birth'] < pd.Timestamp('now'), testing['Date.of.Birth'] -  np.timedelta64(100, 'Y'))

testing['Age']=(pd.Timestamp('now')-testing['Date.of.Birth']).astype('<m8[Y]').astype(int)
testing['Disbursal_months'] = ((pd.Timestamp('now') - testing['DisbursalDate'])/np.timedelta64(1,'M')).astype(int)

#print(testing.Age)

#print(testing.Disbursal_months)
...
training['AVERAGE.ACCT.AGE']=training['AVERAGE.ACCT.AGE'].apply(lambda x: extract_numbers(x))
training['CREDIT.HISTORY.LENGTH']=training['CREDIT.HISTORY.LENGTH'].apply(lambda x: extract_numbers(x))
...
testing['AVERAGE.ACCT.AGE']=testing['AVERAGE.ACCT.AGE'].apply(lambda x: extract_numbers(x))
testing['CREDIT.HISTORY.LENGTH']=testing['CREDIT.HISTORY.LENGTH'].apply(lambda x: extract_numbers(x))

#Univariate Analysis
import matplotlib.pyplot as plt # Charge matplotlib
import seaborn as sns   # Charge seaborn

##To have a from all the variables
def ploteo_masive(variables):
    cadena='''sns.displot(training['{}'][training['{}'] < training['{}'].quantile(.99)],kind='hist',kde=True)'''
    for i in variables:
        exec(cadena.format(i,i,i))
        plt.show()
            
Columns_to_plot=['disbursed_amount','asset_cost','ltv','PERFORM_CNS.SCORE','PRI.NO.OF.ACCTS','PRI.ACTIVE.ACCTS',
'PRI.OVERDUE.ACCTS',
'PRI.CURRENT.BALANCE',
...
ploteo_masive(Columns_to_plot)

# Traditional Credit Scoring Using Logistic Regression
#!pip install scorecardpy

import scorecardpy as sc
def ploteo_masive_woe(variables):
    cadena1='''bins_{}= sc.woebin(training, y = "loan_default",x = ["{}"])'''
    cadena2='''sc.woebin_plot(bins_{})'''
#    cadena3='''pd.concat(bins_{})'''
#    cadena4='''list_break=pd.concat(bins_{}).breaks.astype('float').to_list()'''
#    cadena5='''list_break.insert(0,float('-inf'))'''
#    cadena6='''training['{}_new'] = pd.cut(training["{}"],list_break)'''
    
    for i in variables:
        j=i.replace(".","")
        print(i,j)
        exec(cadena1.format(j,i))
        exec(cadena2.format(j))
#       exec(cadena3.format(j))
#       exec(cadena4.format(j))
#       exec(cadena5)
#       exec(cadena6.format(j,i))
        plt.show()
            
Columns_to_plot_woe=['disbursed_amount',
                     'asset_cost',
                     'ltv',
                     'Passport_flag',
                    'Aadhar_flag',
...
ploteo_masive_woe(Columns_to_plot_woe)

#1
#bins_disbursed_amount= sc.woebin(training, y = "loan_default",x = ["disbursed_amount"])
...
#training['disbursed_amount_new'] = pd.cut(training["disbursed_amount"],list_break)
#print(training[['disbursed_amount','disbursed_amount_new']])
#training.head()

training['branch_id']=training['branch_id'].astype('object')
training['supplier_id']=training['supplier_id'].astype('object')
training['manufacturer_id']=training['manufacturer_id'].astype('object')
training['Current_pincode_ID']=training['Current_pincode_ID'].astype('object')
training['State_ID']=training['State_ID'].astype('object')
training['Employee_code_ID']=training['Employee_code_ID'].astype('object')
training['MobileNo_Avl_Flag']=training['MobileNo_Avl_Flag'].astype('object')
training['Aadhar_flag']=training['Aadhar_flag'].astype('object')
training['PAN_flag']=training['PAN_flag'].astype('object')
training['VoterID_flag']=training['VoterID_flag'].astype('object')
training['Driving_flag']=training['Driving_flag'].astype('object')
training['Passport_flag']=training['Passport_flag'].astype('object')
training['PERFORM_CNS.SCORE.DESCRIPTION']=training['PERFORM_CNS.SCORE.DESCRIPTION'].astype('object')
#df['loan_default']=df['loan_default'].astype('object')

#Building a Dataset for Categorical (continous)
training_categorical = training.select_dtypes(exclude=['number'])
training_categorical=training_categorical.drop(['Date.of.Birth','DisbursalDate'],axis=1)
training_categorical.head()

#Building a Dataset for numerical (continous)
training_continuous = training.select_dtypes(include=['number'])
training_continuous=training_continuous.drop(['UniqueID'],axis=1)
training_continuous.head()

#To obtain the basic statistics
training_continuous.describe()

#Get the List of all Column Names
continuous_list = list(training_continuous)
print(continuous_list)

########Multivariate Analysis

plt.rcParams["figure.figsize"] = (10,6)
sns.heatmap(training_continuous.corr())
plt.show()

#Heat map
sns.heatmap(training_continuous.corr(), cmap="YlGnBu", annot=False,mask=np.triu(training_continuous.corr()))
plt.show()

#Heat map that highligts if the correlation is greater than 0.6
sns.heatmap(training_continuous.corr().abs()>0.6, cmap="YlGnBu", annot=False,mask=np.triu(training_continuous.corr()))
plt.show() # black are with the highest correlation

def get_redundant_pairs(training):
    '''Get diagonal and lower triangular pairs of correlation matrix'''
    pairs_to_drop = set()
    cols = training.columns
    for i in range(0, training_continuous.shape[1]):
        for j in range(0, i+1):
            pairs_to_drop.add((cols[i], cols[j]))
    return pairs_to_drop

def get_top_abs_correlations(training, n=5):
    au_corr = training.corr().abs().unstack()
    labels_to_drop = get_redundant_pairs(training_continuous)
    au_corr = au_corr.drop(labels=labels_to_drop).sort_values(ascending=False)
    return au_corr[0:n]

list1=get_top_abs_correlations(training_continuous,n=9)
print(list1)
...
#Normalization to the variables 

from sklearn.preprocessing import MinMaxScaler
scaler = MinMaxScaler()

# ltv
training['ltv_nor']=scaler.fit_transform(training[['ltv']])
testing['ltv_nor']=scaler.fit_transform(testing[['ltv']])

training[['ltv','ltv_nor']].describe()

#Age
training['Age_nor']=scaler.fit_transform(training[['Age']])
testing['Age_nor']=scaler.fit_transform(testing[['Age']])

training[['Age','Age_nor']].describe()

training.shape #45

testing.shape #45

# Fill the NA with a category

training['Employment.Type']=training['Employment.Type'].fillna('Without_job')
testing['Employment.Type']=testing['Employment.Type'].fillna('Without_job')

pd.value_counts(training['Employment.Type'])

pd.value_counts(training['PERFORM_CNS.SCORE.DESCRIPTION'])

# Coding the categorical variables
training=pd.get_dummies(training, columns=['Employment.Type'])
testing=pd.get_dummies(testing, columns=['Employment.Type'])

training.shape

training['PERFORM_CNS.SCORE.DESCRIPTION'].value_counts()

testing['PERFORM_CNS.SCORE.DESCRIPTION'].value_counts()

training["PERFORM_CNS.SCORE.DESCRIPTION"].replace({"Not Scored: More than 50 active Accounts found": "Not Scored", "Not Scored: Only a Guarantor": "Not Scored"}, inplace=True)
testing["PERFORM_CNS.SCORE.DESCRIPTION"].replace({"Not Scored: More than 50 active Accounts found": "Not Scored", "Not Scored: Only a Guarantor": "Not Scored"}, inplace=True)

training['PERFORM_CNS.SCORE.DESCRIPTION'].value_counts()

training=pd.get_dummies(training, columns=['PERFORM_CNS.SCORE.DESCRIPTION'])
testing=pd.get_dummies(testing, columns=['PERFORM_CNS.SCORE.DESCRIPTION'])

training.shape

testing.shape

training.columns = training.columns.str.strip().str.lower().str.replace(' ', '_').str.replace('(', '').str.replace(')', '').str.replace('-', '_').str.replace(':', '_')
testing.columns = testing.columns.str.strip().str.lower().str.replace(' ', '_').str.replace('(', '').str.replace(')', '').str.replace('-', '_').str.replace(':', '_')

training.info()

testing.info()

del training['uniqueid'] 
del testing['uniqueid'] 
#training.info()

y_train=training['loan_default'] # variable to be predicted in TRAIN
X_train=training.drop(['loan_default','date.of.birth','disbursaldate'],axis=1) # predictor variables in TRAIN


y_test=testing['loan_default'] # variable to be predicted in TEST
X_test=testing.drop(['loan_default','date.of.birth','disbursaldate'],axis=1) # predictor variables in TRAIN
...
metrics.plot_confusion_matrix(lr, X_train.iloc[:,list_rf], y_train)
plt.show()

metrics.plot_confusion_matrix(lr, X_test.iloc[:,list_rf], y_test)
plt.show()
...
##### Build buckets for some variables
